...
def test_right_odd_source_page_uses_odd_shift_value():
    rect = fitz.Rect(0, 0, 210, 297)
    placement = placement_for_page(rect, 100.0, ShiftSettings(11.0, 15.0), PageSide.RIGHT_ODD, BindingSide.LEFT)
    assert placement.shift_mm == 11.0


def test_left_even_source_page_uses_even_shift_value():
    rect = fitz.Rect(0, 0, 210, 297)
    placement = placement_for_page(rect, 100.0, ShiftSettings(11.0, 15.0), PageSide.LEFT_EVEN, BindingSide.LEFT)
    assert placement.shift_mm == 15.0


def test_left_binding_directions_remain_correct():
    assert page_shift_sign(PageSide.RIGHT_ODD, BindingSide.LEFT) == 1
    assert page_shift_sign(PageSide.LEFT_EVEN, BindingSide.LEFT) == -1


def test_right_binding_directions_remain_correct():
    assert page_shift_sign(PageSide.RIGHT_ODD, BindingSide.RIGHT) == -1
    assert page_shift_sign(PageSide.LEFT_EVEN, BindingSide.RIGHT) == 1


def test_equal_shift_mode_keeps_values_synchronized():
    shifts = ShiftSettings(5.0, 5.0)
    assert shifts.is_uniform() is True
    assert shifts.mm_for_side(PageSide.RIGHT_ODD) == 5.0
    assert shifts.mm_for_side(PageSide.LEFT_EVEN) == 5.0


def test_unequal_shifts_are_passed_into_geometry():
    rect = fitz.Rect(0, 0, 210, 297)
    left_page = placement_for_page(rect, 100.0, ShiftSettings(11.0, 15.0), PageSide.RIGHT_ODD, BindingSide.LEFT)
    right_page = placement_for_page(rect, 100.0, ShiftSettings(11.0, 15.0), PageSide.LEFT_EVEN, BindingSide.LEFT)
